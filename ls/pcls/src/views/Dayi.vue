		  ...
		  <div class="tvwrap w12">
				<div class="tvwrapleft comborder">
					<h2 class="comtitle">
						...
						<span>当前位置：<a href="">北京冠领律师事务所拆迁官网</a> > <router-link :to="{path:breadcrumbList[0]['path']}">{{breadcrumbList[0]['name']}}</router-link> ></span> 
					</h2>
					<ul>
	...
	export default{
		...
		methods:{
			getData(){
				let that = this
				request({
					url: '/dayi/read?id=44&page=1&page_size=20',
					responseType: 'json',
			...
			getPage(val){
				let that = this
				request({
					url: '/dayi/fagui?&page='+val+'&page_size=20',
					responseType: 'json',
					transformResponse:[function(data){
						let jsondata = JSON.parse(data)
						// console.log(jsondata);
			...
			toinfo(id,catid){
				...
				if(id == 'dayi'){
					this.$router.push({ path:'/dayi/list/index.html'})
				}else{
					this.$router.push({ path:'/dayi/'+id+'.html'})
				}
			}
		},
		mounted() {
			...
			let dayicatid = localStorage.getItem('dayicatid')
