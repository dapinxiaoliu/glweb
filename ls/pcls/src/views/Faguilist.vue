		  ...
		  <div class="tvwrap w12">
				<div class="tvwrapleft comborder">
					<h2 class="comtitle">
						...
						<span>当前位置：<a href="">北京冠领律师事务所拆迁官网</a> > <a href='javascript:void(0)' @click="toinfo('faguix',61)">{{breadcrumbList[0]['name']}}</a> > <a href="javascript:void(0)">{{lmname}}</a> ></span> 
					</h2>
					<ul>
						<li v-for="item,index in anliData" :key="item.id">
							<a href="javascript:void(0)" @click="toinfo('fagui',item.id,item.catid)">{{item.title}}</a><span>{{item.create_time}}</span>
						</li>
	...
	export default{
		...
		methods:{
			getData(){
				let that = this
				request({
					url: '/dayi/read?id='+that.lmid+'&page=1&page_size=20',
					responseType: 'json',
			...
			toinfo(id,catid,cid){
				...
				if(id == 'fagui'){
					this.$router.push({ path:'/fagui/'+catid+'.html'})
				}else{
		...
		mounted() {
			...
			let faguicatid = localStorage.getItem('faguicatid')
			// let anlinames = localStorage.getItem('anlinames')
			// alert(anlinames)
			this.lmid = faguicatid
