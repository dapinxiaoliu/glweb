		...
		<div class="tvwrap w12">
			<div class="tvwrapleft">
				<div class="tvwrapleftitem comborder">
					...
					<ul>
						<li v-for="item,index in jinqiData" :key="item.id"><a href="javascript:void(0)" @click="toinfo(item.id,item.catid)">{{item.title}}</a><span>{{item.create_time}}</span></li>
					</ul>
				</div>
				<div class="tvwrapleftitem comborder">
					<h2 class="comtitle">
					...
					<ul>
						<li v-for="item,index in jiangbeiData" :key="item.id"><a href="javascript:void(0)" @click="toinfo(item.id,item.catid)">{{item.title}}</a><span>{{item.create_time}}</span></li>
					</ul>
				</div>
	...
	export default{
		...
		methods:{
			getData(lmid){
				...
				request({
					url: '/honor/read?id='+lmid+'&page=1&page_size=10',
					responseType: 'json',
					transformResponse:[function(data){
						...
						if(jsondata['code'] == 200){
							...
							beforeData['data'].forEach(function(value,index,arr){
								...
								if(lmid == 24){
									that.jinqiData.push(value)
								}else if(lmid == 25){
									that.jiangbeiData.push(value)
								}
							})
						}
					}]
